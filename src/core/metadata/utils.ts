...
class MetadataHelper {
  ...
  public static getMetaData(target: Object, decorator?: string, propertyKey?: string, paramIndex?: number): any {
    switch (arguments.length) {
      case 1:
        return MetadataHelper.getMetaDataForTarget(target);
      case 2:
        return MetadataHelper.getAllMetaDataForDecorator(target, decorator);
      case 3:
        return MetadataHelper.getMetaDataForTargetDecoratorAndPropKey(DecoratorType.METHOD, target, decorator, propertyKey, paramIndex);
      case 4:
        return MetadataHelper.getMetaDataForTargetDecoratorAndPropKey(DecoratorType.PARAM, target, decorator, propertyKey, paramIndex);
    }
  }
  ...
  private static getMetaDataForTargetDecoratorAndPropKey(decoratorType: DecoratorType,
                                                         target: Object,
                                                         decorator: string,
                                                         propertyKey: string,
                                                         paramIndex?: number): MetaData {
    if (!target || !decorator) {
      winstonLog.logError('target and decorator cannot be null or undefined');
      throw TypeError('target and decorator cannot be null or undefined');
    }
